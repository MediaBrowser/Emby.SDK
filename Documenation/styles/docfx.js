...
$(function () {
  ...
  ////window.setTimeout(enableSearch, 2000);
  enableSearch();
  ...
  function enableSearch() {
    ...
    function localSearch() {
        ...
        var searchIndex1 = searchIndex;
        var compiledIndex1 = compiledIndex;
        lunr.tokenizer.seperator = /[\s\-\.]+/;

        var lunrIndex = null;
        var searchData = null;
        var compiledData = null;
        ...
        $("body").bind("queryReady",
            function() {
                ...
                if (!lunrIndex) {
                    ...
                    var dec1 = lzw_decode(searchIndex1);
                    var dec2 = lzw_decode(compiledIndex1);

                    searchData = JSON.parse(dec1);
                    compiledData = JSON.parse(dec2);

                    lunrIndex = lunr.Index.load(compiledData);
                }
  ...
  function renderNavbar() {
    ...
    if (typeof (navbar) === 'undefined') {
      loadNavbar();
        ...
        // Second attempt - more relaxed matching
        $('#navbar').find('a[href]').each(function(i, e) {
            ...
            var href = $(e).attr("href");
            var navRootPath = href.replace(navrel, '').split('/')[0];

            let levels = getParentPathLevels(navrel);
            let pathParts = splitPath(currentAbsPath);

            if (levels >= pathParts.length) {
                return;
            }
            ...
            if (pathParts[pathParts.length - 1] === navRootPath) {
                $(e).addClass(active);
                foundActive = true;
            }
        });
    ...
    function getParentPathLevels(path) {
        let i = 0;

        while (path && path.length > 2 && path.slice(0, 3) === '../') {
            path = path.slice(3);
            i++;
        }
    ...
    function loadNavbar() {
      ...
      if (typeof navData !== 'undefined') {
          var htmlStr = atob(navData);

          $(htmlStr).find("#toc>ul").appendTo("#navbar");
          showSearch();
          var index = navbarPath.lastIndexOf('/');
          var navrel = '';
          if (index > -1) {
          ...
          $('#navbar').find('a[href]').each(function(i, e) {
              ...
              if (util.isRelativePath(href)) {
                  ...
                  if (originalHref) {
                      originalHref = navrel + originalHref;
                      if (util.getDirectory(util.getAbsolutePath(originalHref)) ===
  ...
  function renderSidebar() {
    ...
    function loadToc() {
      ...
      if (typeof tocData !== 'undefined') {
          var htmlStr = atob(tocData);
          $('#sidetoc').html(htmlStr);
  ...
  function renderBreadcrumb() {
    ...
    if (breadcrumb.length > 1 && breadcrumb[0].href === breadcrumb[1].href) {
